...
#error Unsupported platform
    ...
    void GenerateCrashDump(DWORD dwThreadId, EXCEPTION_POINTERS *ExceptionInfo)
    {
        ...
        TCHAR szDumpFilePath[1024];
        ::GetTempPath(1024, szDumpPath);
        srand((unsigned int)time(NULL));
        ::GetTempFileName(szDumpPath, _T("soya"), rand(), szDumpFilePath);

        HANDLE hFile = ::CreateFile(szDumpFilePath, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, 0, 0);
        ...
        CallPostWriteDump(szDumpFilePath);
    }
...
#define STATUS_PURE_CALL         (0xC137000DL)
...
#define STATUS_SIGNAL_ABORT         (0xC137000EL)
#endif  /* STATUS_SIGNAL_ABORT */
#ifndef STATUS_SIGNAL_TERM
#define STATUS_SIGNAL_TERM         (0xC137000FL)
...
#define STATUS_NEW_FAILED         (0xC1370010L)
...
{
    void StartCrashClient(IN PrevWriteDumpCallback prev, IN PostWriteDumpCallback post)
    {
